...
fun filterNazwa(listaInput: MutableList<Panstwo>, listaOutput: MutableList<Panstwo>, nazwaKlucz: String?){
    for(i in listaInput){
        var name = i.n.lowercase()
        if(nazwaKlucz?.let { name.contains(it) } == true || name == "" || name == " "|| name == null){
            listaOutput.add(i)
        }
    }
}